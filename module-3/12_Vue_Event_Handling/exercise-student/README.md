...
- When the `selectedUserIDs` array is empty, the buttons should be disabled.
  ...
  - When the checkbox for a row is checked, add the user's ID to the `selectedUserIDs` array. 
    >(HINT: When the checkbox for a row is unchecked, remove the user's ID from the `selectedUserIDs` array)
  - When the checkbox for a row is checked, add the user's ID to the `selectedUserIDs` array.
    > (HINT: When the checkbox for a row is unchecked, remove the user's ID from the `selectedUserIDs` array)
    - Do this with a `selectUser()` method that is passed the event parameter. 
  > (HINT: Use the filter method to remove the ID when the row is unchecked)
...
- Enable Users
  ...
  >(HINT: loop through the users array checking if the id is included in the SelectedUserIDs array.  If it is, change the status)
...
- Delete Users
  ...
  >(HINT: for each ID in the SelectedUserIDs array, use the filter function to remove them from the users array)
  - Clears all checkboxes when action is completed.
...
>(HINT: create a method called selectAll() that receives the event (so you can determine if it is checked or unchecked).  If it is checked, add all ids to the selectedUserIDs array.  If it is unchecked, set the selectedUserIDs array to empty)
