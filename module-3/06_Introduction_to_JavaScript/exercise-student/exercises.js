      ...
      function oddOnly(arrayOfInt){
		...
		var even=[];
		   for(i=0; i<arrayOfInt.length; i++)
		   {
				   if(arrayOfInt[i] % 2==0 )
				   {
	  				 even.push(arrayOfInt[i])
	  			 	}
		 		   else {
	 				  odd.push(arrayOfInt[i])
		   			}
		   }
		   return odd
	  }
...
*/
	  function cigarParty(numCigar, isWeekEnd){
		  if((numCigar >=40 && numCigar <=60) && !(isWeekEnd)){
			  return true
		  } else if(numCigar >=40 && isWeekEnd){
			  return true
		  } else{
			  return false
		  }
...
*/
    function filterMultiplesOfX(numbers, x){
		...
		let  multipleOfParameter = []; 
  		for (let i = 0; i < numbers.length; i++) {
   			 if (numbers[i] % x === 0) { 
				multipleOfParameter.push(numbers[i]); 
    		 }
  		}
		  return multipleOfParameter;
	}
