...
public class Exercises {
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		...
		if(peterMoney > 0 && paulMoney < 1000){
			peterPaul.put("Peter", peterMoney - (peterMoney /2));
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		...
		int peterMoney = peterPaul.get("Peter");
		int paulMoney = peterPaul.get("Paul");
		int paulPeterPartnershipMoney = 0;

		if(peterMoney >= 5000 && paulMoney>=10000){
			int peterQuarterWorth = peterMoney / 4;
			peterMoney -= peterQuarterWorth;
			int paulQuarterWorth = paulMoney / 4;
			paulMoney -= paulQuarterWorth;
			paulPeterPartnershipMoney = peterQuarterWorth + paulQuarterWorth;
			peterPaul.put("Peter", peterMoney);
			peterPaul.put("Paul", paulMoney);
			peterPaul.put("PeterPaulPartnership",paulPeterPartnershipMoney );
			return peterPaul;
		}
		return peterPaul;
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		...
		for (String word : words) {
			beginningAndEnding.put(word.substring(0, 1), word.substring(word.length() - 1, word.length()));
		}
	...
	public Map<Integer, Integer> integerCount(int[] ints) {
		...
		for (int integerThatAppears : ints) {
			if(integerCount.containsKey(integerThatAppears)) {
				int count = integerCount.get(integerThatAppears);
				count++;
				integerCount.put(integerThatAppears, count) ;
			} else {
				integerCount.put(integerThatAppears, 1);
			}
		}
