...
*/
	...
	public String stringTimes(String str, int n) {
		...
		String copyOrigString= "";

		for (int i=0; i<n; i++) {
			copyOrigString += str;
		}
		return copyOrigString;
	}
	...
	public String frontTimes(String str, int n) {
		...
		if(str.length()<4){
			...
			for(int i=0; i<n; i++){
				copyOrigString +=str.substring(0,3);
			}
		}
		return copyOrigString;
	}
	...
	public boolean doubleX(String str) {
		...
		int i = str.indexOf("x");
		//check if x is present in the given string; returns false if x there's no x
		if (i == -1){
			return false;
		}

		// Check if the next char at position i+1 is also an "x"?
		if (i+1 >= str.length()) {
			return false;
		}
	...
	public String stringX(String str) {
		...
		String withOutXVersion = "";

		for (int i = 0; i < str.length(); i++){
			char temp = str.charAt(i);
			if (!(i > 0 && i < str.length()- 1 && temp == 'x'))
				withOutXVersion = withOutXVersion + temp;
		}
		return withOutXVersion;
