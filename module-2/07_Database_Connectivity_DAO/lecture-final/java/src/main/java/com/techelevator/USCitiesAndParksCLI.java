...
public class USCitiesAndParksCLI {
    ...
    private final Scanner userInput = new Scanner(System.in); // instantiate a Scanner class for user input use

    private final StateDao stateDao; // create an instance variable of interface type ? StateDao - for whatpurpose?
    private final CityDao cityDao; // create an instance variable of interface type ? CityDao - for whatpurpose?
    private final ParkDao parkDao; // create an instance variable of interface type ? ParkDao - for whatpurpose?

    public static void main(String[] args) {
        //create the connection;
        BasicDataSource dataSource = new BasicDataSource(); // how is the imported; responsible for creating connection to the database
        dataSource.setUrl("jdbc:postgresql://localhost:5432/UnitedStates");// sets the network location of the database
                        //could be a local connection to a database on your own workstation
        ...
        USCitiesAndParksCLI application = new USCitiesAndParksCLI(dataSource); //why is it that we have to create an
                                     // object of this main app class// oh we are passing the datasource as a parameter
        // JDBC template’s constructor requires a data source

        application.run(); //call the run method
    }
