...
public class EmployeeProjectsCLI {
	...
	private void handleEmployeeProjectRemoval() {
		...
		if(projectEmployees.size() > 0) {
			...
			employeeDao.removeEmployeeFromProject(selectedProject.getId(), selectedEmployee.getEmployeeId());
	...
	private void handleEmployeeProjectAssignment() {
		...
		employeeDao.addEmployeeToProject(selectedProject.getId(), selectedEmployee.getEmployeeId());
