...
public class JdbcTimesheetDao implements TimesheetDao {
    ...
    public List<Timesheet> getTimesheetsByEmployeeId(Long employeeId) {
        ...
        while(results.next()) { // originally an if and changed
    ...
    public double getBillableHours(Long employeeId, Long projectId) {
        ...
        String sql = "SELECT SUM(hours_worked) AS billable_hours " + "FROM timesheet " +
                     "WHERE employee_id = ? AND project_id = ? AND billable = true "; // added AND billable = true
