...
SELECT state_name, COUNT(*) AS number_of_cities
    FROM city c
    JOIN state ON c.state_abbreviation = state.state_abbreviation
    WHERE state.census_region = 'Midwest'
    GROUP BY state_name
...
-- Modify the previous query so the results include entries for all the records in the state table, even if 
--  they have no park records associated with them.
SELECT state_name, MIN(date_established) AS earliest_park
   FROM state
...
-- make sure it is selected in DbVisualizer and confirm it is working correctly by writing queries to retrieve...
